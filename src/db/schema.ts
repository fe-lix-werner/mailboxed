...
export const mailboxes = sqliteTable("mailboxes", {
  ...
  tlsMode: text("tls_mode", { enum: ["tls", "starttls", "none"] }).notNull().default("tls"),
  ...
  syncMode: text("sync_mode", { enum: ["everything", "from-now-on"] }).notNull().default("everything"),
  enabled: integer("enabled", { mode: "boolean" }).notNull().default(true),
  pollIntervalSec: integer("poll_interval_sec").notNull().default(600),
  busyPolicy: text("busy_policy", { enum: ["skip", "queue_one"] }).notNull().default("skip"),
  createdAt: text("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
...
export const checkpoints = sqliteTable("checkpoints", {
  ...
  mailboxFolderIdx: index("mailbox_folder_idx").on(table.mailboxId, table.folder),
}));
...
export const downloads = sqliteTable("downloads", {
  ...
  dedupeIdx: index("dedupe_idx").on(table.mailboxId, table.folder, table.messageUid, table.attachmentPartId),

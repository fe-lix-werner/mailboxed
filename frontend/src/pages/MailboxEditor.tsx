...
export default function MailboxEditor() {
	...
	const {
		...
		values: mailbox
			? {
					...mailbox,
					allowedExtensions: mailbox.filtersJson
						? JSON.parse(mailbox.filtersJson).extensions?.join(", ") || ""
						: "",
					allowedMimes: mailbox.filtersJson
						? JSON.parse(mailbox.filtersJson).mimes?.join(", ") || ""
						: "",
				}
			: {
					name: "",
		...
		mutationFn: (data: any) => {
			const payload = {
				...
				filtersJson: JSON.stringify({
					extensions: data.allowedExtensions
						? data.allowedExtensions
								.split(",")
								.map((s: string) => s.trim())
								.filter(Boolean)
						: [],
					mimes: data.allowedMimes
						? data.allowedMimes
								.split(",")
								.map((s: string) => s.trim())
								.filter(Boolean)
			...
			return isNew
				? api.mailboxes.create(payload)
				: api.mailboxes.update(parseInt(id!), payload);
		},
		onSuccess: () => {
	...
	const setSuggestion = (type: "pdf" | "images" | "docs" | "reset") => {
		if (type === "pdf") {
			...
			setValue(
				"allowedMimes",
				"application/msword, application/vnd.openxmlformats-officedocument.*, application/pdf",
			);
		} else if (type === "reset") {
		...
		<div className="p-10 max-w-5xl mx-auto space-y-10 pb-20">
			...
			<header className="flex items-center gap-6">
				...
				>
					<ChevronLeft
						size={24}
						className="text-slate-400 group-hover:text-slate-900 transition-colors dark:text-slate-500 dark:group-hover:text-white"
					/>
				</button>
				<div>
					...
					<div className="lg:col-span-2 space-y-8">
						...
						<section className="card">
							<div className="px-8 py-5 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3 dark:border-slate-800 dark:bg-slate-800/50">
								...
								<h2 className="font-bold text-slate-900 uppercase tracking-wider text-xs dark:text-slate-300">
									{t("editor.connectionSettings")}
								</h2>
							</div>
							<div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
								<div className="md:col-span-2">
									<label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 dark:text-slate-400">
										{t("editor.displayName")}
									</label>
									<input
										{...register("name", { required: t("editor.required") })}
										className={`input h-12 text-lg font-medium ${errors.name ? "border-red-500 focus:ring-red-500" : ""}`}
										placeholder="Personal Gmail, Office 365..."
									/>
									{errors.name && (
								...
								<div>
									...
									<input
										{...register("host", { required: t("editor.required") })}
										className={`input h-11 ${errors.host ? "border-red-500 focus:ring-red-500" : ""}`}
								...
								<div className="grid grid-cols-2 gap-4">
									<div>
										...
										<input
											type="number"
											{...register("port", {
												required: t("editor.required"),
												valueAsNumber: true,
											})}
											className={`input h-11 ${errors.port ? "border-red-500 focus:ring-red-500" : ""}`}
										/>
										{errors.port && (
											<p className="mt-1 text-xs text-red-500 font-bold">
								...
								<div>
									...
									<input
										...
										className={`input h-11 ${errors.username ? "border-red-500 focus:ring-red-500" : ""}`}
								...
								<div>
									...
									<input
										type="password"
										{...register("password", {
											required: isNew ? t("editor.required") : false,
										})}
										className={`input h-11 ${errors.password ? "border-red-500 focus:ring-red-500" : ""}`}
										placeholder={
											isNew ? "••••••••" : t("editor.passwordPlaceholder")
										}
									/>
									{errors.password && (
										<p className="mt-1 text-xs text-red-500 font-bold">
								...
								<div className="md:col-span-2 pt-4">
									<button
										...
										className={`w-full h-11 flex items-center justify-center gap-2 rounded-xl font-bold text-sm transition-all border-2 ${
											testStatus === "success"
												? "bg-green-50 border-green-200 text-green-700 dark:bg-green-950/30 dark:border-green-900/30 dark:text-green-400"
												: testStatus === "error"
													? "bg-red-50 border-red-200 text-red-700 dark:bg-red-950/30 dark:border-red-900/30 dark:text-red-400"
									...
									>
										...
										{testStatus === "loading"
											? t("editor.testing")
											: testStatus === "success"
												? t("editor.connectionSuccessful")
												: testStatus === "error"
													? t("editor.connectionFailed")
						...
						<section className="card">
							<div className="px-8 py-5 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3 dark:border-slate-800 dark:bg-slate-800/50">
								...
								<h2 className="font-bold text-slate-900 uppercase tracking-wider text-xs dark:text-slate-300">
									{t("editor.storageFilters")}
								</h2>
							</div>
							<div className="p-8 space-y-8">
								<div>
									<label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 dark:text-slate-400">
										{t("editor.baseDownloadPath")}
									</label>
									<input
										{...register("basePath")}
										className="input h-11 font-mono text-sm"
										placeholder="e.g. invoices/primary"
									/>
								...
								<div className="pt-6 border-t border-slate-100 dark:border-slate-800">
									<div className="flex items-center justify-between mb-4">
										<label className="block text-xs font-bold text-slate-500 uppercase tracking-widest dark:text-slate-400">
											{t("editor.filters")}
										</label>
										<div className="flex gap-1.5">
											<button
												type="button"
												onClick={() => setSuggestion("pdf")}
												className="text-[10px] font-bold px-2 py-1 rounded bg-slate-100 text-slate-600 hover:bg-primary-50 hover:text-primary-600 transition-colors dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-primary-900/40 dark:hover:text-primary-400"
											>
												{t("editor.suggestPdf")}
											</button>
											<button
												type="button"
												onClick={() => setSuggestion("images")}
												className="text-[10px] font-bold px-2 py-1 rounded bg-slate-100 text-slate-600 hover:bg-primary-50 hover:text-primary-600 transition-colors dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-primary-900/40 dark:hover:text-primary-400"
											>
												{t("editor.suggestImages")}
											</button>
											<button
												type="button"
												onClick={() => setSuggestion("docs")}
												className="text-[10px] font-bold px-2 py-1 rounded bg-slate-100 text-slate-600 hover:bg-primary-50 hover:text-primary-600 transition-colors dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-primary-900/40 dark:hover:text-primary-400"
											>
												{t("editor.suggestDocs")}
											</button>
											<button
												type="button"
												onClick={() => setSuggestion("reset")}
												className="text-[10px] font-bold px-2 py-1 rounded bg-slate-100 text-slate-600 hover:bg-red-50 hover:text-red-600 transition-colors dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-red-900/40 dark:hover:text-red-400"
											>
												{t("editor.suggestReset")}
											</button>
										</div>
									</div>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
										<div>
											...
											/>
											<p className="mt-1.5 text-[10px] text-slate-400 font-medium italic dark:text-slate-600">
												{t("editor.allowedExtensionsHint")}
											</p>
										</div>
										<div>
											<label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 dark:text-slate-500">
												{t("editor.allowedMimes")}
											</label>
											<input
												{...register("allowedMimes")}
												className="input h-11 text-sm"
												placeholder="application/pdf, image/*..."
											/>
											<p className="mt-1.5 text-[10px] text-slate-400 font-medium italic dark:text-slate-600">
												{t("editor.allowedMimesHint")}
											</p>
										</div>
									</div>
								</div>
					...
					<div className="space-y-8">
						...
						<section className="card">
							...
							<div className="p-8 space-y-6">
								<div>
									<label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 dark:text-slate-400">
										{t("editor.initialSyncMode")}
									</label>
									<div className="grid grid-cols-1 gap-2">
										<label
											className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all cursor-pointer ${watch("syncMode") === "from_now_on" ? "border-primary-500 bg-primary-50/50 dark:bg-primary-950/20" : "border-slate-100 hover:border-slate-200 dark:border-slate-800 dark:hover:border-slate-700"}`}
										>
											<input
												type="radio"
												value="from_now_on"
												{...register("syncMode")}
												className="w-4 h-4 text-primary-600 border-slate-300 focus:ring-primary-500 dark:border-slate-700 dark:bg-slate-900"
											/>
											<div>
												<span className="block text-sm font-bold text-slate-900 dark:text-slate-200">
													{t("editor.fromNowOn")}
												</span>
												<span className="text-[10px] text-slate-500 font-medium leading-none dark:text-slate-500">
													{t("editor.fromNowOnHint")}
												</span>
											</div>
										</label>
										<label
											className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all cursor-pointer ${watch("syncMode") === "everything" ? "border-primary-500 bg-primary-50/50 dark:bg-primary-950/20" : "border-slate-100 hover:border-slate-200 dark:border-slate-800 dark:hover:border-slate-700"}`}
										>
											<input
												type="radio"
												value="everything"
								...
								<div>
									<label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 dark:text-slate-400">
										{t("editor.pollInterval")}
									</label>
									<div className="flex items-center gap-4">
										<input
											type="number"
											{...register("pollIntervalSec", { valueAsNumber: true })}
											className="input h-11 text-center font-bold"
										/>

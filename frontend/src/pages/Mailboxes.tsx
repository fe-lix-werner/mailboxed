  ...
  return (
    <div className="p-10 max-w-7xl mx-auto space-y-10">
      <header className="flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight dark:text-white">{t('mailboxes.title')}</h1>
          <p className="text-slate-500 mt-2 text-lg dark:text-slate-400">{t('mailboxes.subtitle')}</p>
        </div>
        <Link 
          to="/mailboxes/new"
      ...
      <div className="grid grid-cols-1 gap-6">
        {mailboxes?.map((mailbox: any) => (
          <div key={mailbox.id} onClick={() => navigate(`/mailboxes/${mailbox.id}`)} className="cursor-pointer card group p-8 flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h3 className="font-bold text-2xl text-slate-900 dark:text-white">{mailbox.name}</h3>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    toggleMutation.mutate({ id: mailbox.id, enabled: !mailbox.enabled });
                  }}
        ...
        {mailboxes?.length === 0 && (
          <div className="text-center py-24 bg-white rounded-3xl border-2 border-dashed border-slate-200 flex flex-col items-center dark:bg-slate-900 dark:border-slate-800">
            ...
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{t('mailboxes.noMailboxes')}</h3>
            <p className="text-slate-500 mb-8 max-w-sm dark:text-slate-400">{t('mailboxes.connectFirst')}</p>
            <Link 
              to="/mailboxes/new"

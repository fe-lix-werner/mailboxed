	...
	describe("Mailboxes", () => {
		...
		test("POST /api/mailboxes/:id/sync should trigger a sync", async () => {
			...
			const req = new Request(
				`http://localhost/api/mailboxes/${inserted.id}/sync`,
				{
					method: "POST",
					headers: { Cookie: cookie || "" },
				},
	...
	describe("Job Control Endpoints", () => {
		...
		test("Scheduler endpoints should work", async () => {
			...
			const pauseReq = new Request(
				"http://localhost/api/jobs/scheduler/pause",
				{
					method: "POST",
					headers: { Cookie: cookie || "" },
				},
			...
			const statusReq = new Request(
				"http://localhost/api/jobs/scheduler/status",
				{
					headers: { Cookie: cookie || "" },
				},
			);
			...
			const resumeReq = new Request(
				"http://localhost/api/jobs/scheduler/resume",
				{
					method: "POST",
					headers: { Cookie: cookie || "" },
				},
